...
public class AnswerSheet {
    ...
    public void setOptionsMarkCorrects()
    {
        int sumAnswers = 0;
        int countOptions = 1;
        int maxAnswers = this.getNumAnswers();
        ...
        for(int i = 1; i <= this.getNumBlocks(); i++)
        {
            ...
            for (int k = 1; k <= this.getAnswersPerBlock(i); k++)
            {
                ...
                if (indexKey > maxAnswers) {
                    return;
                }
                String correct = this.getCorrect(indexKey);
                Integer index = indexLetter.get(correct);
    ...
    public boolean checkOptionMark(int index)
    {
        ...
        if(resp != null && resp.equals(correct))
        {
            return true;
        }
        return false;
    ...
    public void setResults()
    {
        ...
        for(int i = 1; i <= this.getNumAnswers(); i++)
        {
            ...
            String correct = this.getCorrect(i);
            if(isOpenCorrect(correct))
            {
                continue;
            }
        ...
        String[] letters = new String[this.getNumAnswers()];
        for(int i = 1; i <= this.getNumAnswers(); i++)
        {
            String correct = this.getCorrect(i);
            if (isOpenCorrect(correct)) {
            ...
            String answer = this.answersLetter.get(i);
            if (answer == null || answer.equals("")) {
                letters[(i-1)] = "";
            } else {
                letters[(i-1)] = answer;
        ...
        this.setTotalEvaluated(totalEvaluated);
        if (totalEvaluated > 0) {
            percentageAnswers = Math.round((correctAnswers * 100) / totalEvaluated);
        }
    ...
    private boolean isOpenCorrect(String correct)
    {
        return correct == null || correct.equals("") || correct.equals("X");
    }
    ...
    public boolean isOpenQuestion(int index)
    {
        return isOpenCorrect(this.getCorrect(index));
    }
    ...
    public static ArrayList<RowResult> ANSWERS_TO_ROW(Map<Integer, String> corrects, String answers)
    {
        ...
        for(int i = 1; i <= corrects.size(); i++)
        {
            int index = (i-1);
            String letterAnswer = "";
            if (index < jsonAnswers.size()) {
                letterAnswer = String.valueOf(jsonAnswers.get(index)).replaceAll("\"", "").trim().toUpperCase();
            }
            String letterCorrect = corrects.get(i) != null ? corrects.get(i).toUpperCase() : "";
            boolean isCorrect = false;
            boolean isOpen = letterCorrect.equals("X") || letterCorrect.equals("");
            if(!isOpen && letterAnswer.equals(letterCorrect))
            {
                isCorrect = true;
            }
            ...
            row.id = i;
            row.num = i;
            row.letter = isOpen ? "" : letterAnswer;
